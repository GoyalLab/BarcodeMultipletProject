...
data_dir<-"/projects/p31666/zzhang/doublet-bchmk/data/fatemap_data/FM01/10X/sample1"
...
load_fatemap_into_seurat<-function(data_dir, cell_labels_prefix, doublets_pct=0.08){
  ...
  singlets_pair_for_doublet_simulation_file<-list.files(data_dir, pattern="singlet_pairs.csv")
  ...
  singlet_pair_df<-read.csv(singlets_pair_for_doublet_simulation_file, header = F)|>
    dplyr::mutate_each(funs = function(x) paste(x,"-1",sep = ""))|>
  ...
  singlet_pair_df<-singlet_pair_df[singlet_pair_df$V1 %in% all_seurat_cells &
                                     singlet_pair_df$V2 %in% all_seurat_cells,]

  # take average of expression to produce doublets
  t<-seurat_object[, singlet_pair_df$V1]
  t2<-seurat_object[, singlet_pair_df$V2]
  avg_assay<-(t@assays$RNA@counts+t2@assays$RNA@counts)/2

  # create artificial doublet ID
  doublet_id<-paste(colnames(t), colnames(t2), sep = "--")
  colnames(avg_assay)<-doublet_id
  num_doublets<-length(singlets)*doublets_pct/(1-doublets_pct) |>round()
  ...
  singlet_object<-seurat_object[, singlets]
...
run_doublet_finder<-function(doublet.seurat){
...
run_bchmk<-function(seurat_object, data_sample_ID, num_to_test){
  df_ls<-NULL
  for(idx in 1:num_to_test){
    cur_res<-run_doublet_finder(seurat_object)
...
run_doublet_finder_on_dataset<-function(data_dir, METHOD_ID, num_to_test=5){
  ...
  for (cur_sample_dir in sample_dirs){
    ...
    cur_df <- run_bchmk(cur_seurat_object, cur_data_sample_id, num_to_test)
...
```{r}
cur_out_df<-run_doublet_finder_on_dataset("/projects/p31666/zzhang/doublet-bchmk/data/fatemap_data/FM01", METHOD_ID)
```


```{r}
data_dir = "/projects/p31666/zzhang/doublet-bchmk/data/fatemap_data/FM01/10X/sample2"
doublets_pct = 0.08
